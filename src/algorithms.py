...
class Approximationalgo:
  def __init__(self):
        self.parent = list(np.zeros(100,))
        self.d = list(np.zeros(100,))
        self.size = list(np.zeros(100,))
        self.parent1 = list(np.zeros(100,))
        self.size1 = list(np.zeros(100,))
  ...
  def buildMaximallyLeafyForest(self,noofvertices,G):
      ...
      for i in range(noofvertices):
          ...
          for j in range(len(G[i])):
              u = G[i][j]
              if(self.findparent(u) != self.findparent(v) and [u,self.findparent(u)] not in s1):
                  d1 = d1 + 1
